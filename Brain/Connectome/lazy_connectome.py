...
class LazyConnectome(Connectome):
    ...
    def _calc_prev_inputs(self, area: Area, sources: List[BrainPart]) -> List[float]:
        ...
        for src in sources:
            for i in range(len(area.support_size)):
                prev_inputs[i] = sum((inp[i] for inp in self.connections[src][area].synapses if len(inp) >= i))

        return prev_inputs

    def _calculate_input_sizes(self, sources: List[BrainPart]) -> Dict[BrainPart, int]:
        """
        ...
        """
        input_sizes: Dict[BrainPart, int] = {src: (src.n if isinstance(src, Area) else src.k) for src in sources}
    ...
    def _calc_new_winners(self, area: Area, prev_winner_inputs: List[float], potential_new_winners: List[float]) \
        ...
        new_winners = np.argpartition(both, area.k)
        num_first_winners = 0
        first_winner_inputs = []
        for i in range(area.k):
    ...
    def _project_into(self, area: Area, sources: List[BrainPart]) \
        ...
        logging.info(f'Projecting {", ".join(sources)} into {area}')

        prev_inputs: List[float] = self._calc_prev_inputs(area, sources)
        input_sizes: Dict[BrainPart, int] = self._calculate_input_sizes(sources)
    ...
    def project(self, connections: Dict[BrainPart, List[Area]]):
        ...
        for area_info in areas_info:
            area_info.area.winners = area_info.new_winners[area]
            area_info.area.support_size = area_info.new_support_sizes[area]


###################################### ADI ###########################################

    def calculate_first_winner_to_inputs(self, first_winner_inputs: List[float], input_sizes: Dict[BrainPart, int]) ->  Dict[int, Dict[BrainPart: int]]:
        """
        ...
        for i in range(len(first_winner_inputs)):
            ...
            for source in input_sizes:
                ...
                inputs[source] = sum([(total_so_far <= w < (total_so_far + input_sizes[source])) for w in input_indices])
    ...
    def _build_stim_to_area(self, stim: Stimulus, area: Area, first_winner_to_inputs: Dict[int, Dict[BrainPart, int]]):
        # resize connections stim->area to the new support size
        self.connections[stim, area] = np.resize(self.connections[stim, area],
                                                 area.support_size + len(first_winner_to_inputs))
    ...
    def _build_fired_other_to_area(self,
        ...
        self.connections[other, area] = np.pad(self.connections[other, area],
    ...
    def _build_nonfired_other_to_area(self, other: Area, area: Area, new_support_size: int, first_winner_to_inputs: Dict[int, Dict[BrainPart, int]]):
        # add num_first_winners columns to self.connections[other_area, area]
        self.connections[other, area] = np.pad(self.connectomes[other, area],
                                               ((0, 0), (0, len(first_winner_to_inputs))), 'constant',
                                               constant_values=0)
        for other_neuron in range(self.areas[other].support_size):
    ...
    def _build_area_to_other(self, other: Area, area: Area, new_support_size: int, first_winner_to_inputs: Dict[int, Dict[BrainPart, int]]):
        self.connections[area, other] = np.pad(self.connections[area, other],
    ...
    def _build_area_connections(self, area_info: AreaInfo, first_winner_to_inputs: Dict[int, Dict[BrainPart, int]]) -> None:
        if len(first_winner_to_inputs) == 0: # TODO: Should this be here? I only added this because of the original code
            return

        area, sources, new_winners, new_support_size = area_info.area, area_info.sources,\
        ...
        for source in sources:
            ...
            if isinstance(source, Area):
                self._build_other_to_area(source, area, first_winner_to_inputs)
    ...
    def update_connectomes(self, area_infos: List['LazyConnectome.AreaInfo']) -> None:
        for area_info in area_infos:
            self._build_area_connections(area_info,
                                         self.calculate_first_winner_to_inputs(area_info.input_sizes,
